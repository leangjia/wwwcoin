...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    // wwwcoin start
    if ((nHeight > 1) && (vCommunityFundAddress.size() > 0)) {
        // Find Reward founder address
        size_t i = nHeight % vCommunityFundAddress.size();
        CBitcoinAddress address(vCommunityFundAddress[i].c_str());   
        if(address.IsValid()) {
           // Founders reward is REWARD_PERCENT of the block subsidy
           auto vFoundersReward = coinbaseTx.vout[0].nValue * FUND_REWARD_PERCENT;
           coinbaseTx.vout[0].nValue -= vFoundersReward;
           CScript script = GetScriptForDestination(address.Get());
           // And give it to the founders
           coinbaseTx.vout.push_back(CTxOut(vFoundersReward, script));
        }
    }
    // wwwcoin end
